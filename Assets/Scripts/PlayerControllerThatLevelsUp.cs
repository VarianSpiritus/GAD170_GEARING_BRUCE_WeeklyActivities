...
public class PlayerControllerThatLevelsUp : MonoBehaviour
{
    ...
    // For each level, the player adds 1% to their jump height
    void SetCurrentJumpHeight()
    {
        currentJumpHeight = this.jumpHeight + (this.jumpHeight * 0.01f * level);
        Debug.Log("currentjumpheight = " + currentJumpHeight);
    }
    // For each level, the player adds 1 point to their Lockpick Skill
    void SetCurrentLockPickSkill()
    {
        currentJumpHeight = this.lockPickSkill + level;
